...
describe("Home page", () => {
  ...
  it("redirect to films when click to card film", () => {
    cy.get('[data-cy="movie-card"]', { timeout: 5000 }).first().click();
    cy.get('[data-cy="movie-card-btn"]', { timeout: 5000 }).first().click();
  ...
  it("add trailerID to url params", () => {
    cy.get('[data-cy="slide-in-new-trailer-slide"]', { timeout: 5000 }).first().click();
